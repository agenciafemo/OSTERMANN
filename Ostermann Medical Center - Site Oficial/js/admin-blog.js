(function () {
  ...
  function publishDateTime(post) {
    return new Date(`${post.publishedAt || today()}T${post.publishedAtTime || "00:00"}`);
  }
  ...
  function renderStats() {
    ...
    $("[data-stat-scheduled]").textContent = posts.filter((post) => post.status === "scheduled" || publishDateTime(post) > now).length;
  }
